...
for f, file_path in tqdm(csv_files_info, desc="📄 Procesando CSV", unit="archivo"):
    try:
        ...
        df_head = data.head(2 * nStaWifi).copy()
        
        # Convertimos el último octeto a número para filtrar al AP (nStaWifi + 1)
        last_octet = df_head['SourceAddress'].str.split('.').str[-1].astype(int)
        data_filtered = df_head[last_octet <= nStaWifi].copy()
