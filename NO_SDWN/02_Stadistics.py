...
GroupBy_Columns = [
    'PacketSize', 'nStaWifi', 'nStaH', 'nStaM', 'nStaL', 'nStaNRT',
...
for group_values, group_df in tqdm(grupos, desc="📊 Calculando métricas", unit="grupo"):
    ...
    for priority in Priorities:
        ...
        p_data = priority_df[priority_df['LostPackets'] < 0.99*600].copy()
        
        if not p_data.empty:
            # Aplicar penalización de Timeout al Delay
            p_data['Delay(ms)'] = (p_data['Delay(ms)']*p_data['ReceivedPackets'] + (9*1024*7/1000)* p_data['LostPackets'])/p_data['SentPackets']
            ...
            # --- METRICAS DE PÉRDIDA ---
            loss_pct = (p_data['LostPackets'] / p_data['SentPackets'] * 100)

            weights = p_data['SentPackets']
            weighted_mean = (loss_pct * weights).sum() / weights.sum()

            weighted_var = (weights * (loss_pct - weighted_mean) ** 2).sum() / weights.sum()
            weighted_std = weighted_var ** 0.5
            result_row[f'LostPackets_{sfx}_mean'] = round(weighted_mean, 5)
            result_row[f'LostPackets_{sfx}_std'] = round(weighted_std, 5)
            result_row[f'LostPackets_{sfx}_count'] = p_data['LostPackets'].sum()
            result_row[f'SendPackets_{sfx}_count'] = p_data['SentPackets'].sum()
